 ...
 if (!empty($_POST)) {
   ...
   // $sql = 'SELECT * FROM `tweets`;'; 
    ...
    while ($tweet = mysqli_fetch_assoc($tweets)) {
      ...
      // いいねが付いている　tweet_id と　member_id が一致するものを探す。
      // likeの数を数える。
      $sql = 'SELECT COUNT(*) AS `like_flag` FROM `likes` where `tweet_id` = '.$tweet['tweet_id'].' AND `member_id` = '.$_SESSION['login_member_id'];
       $likes = mysqli_query($db,$sql) or die(mysqli_error($db));
        ...
        // echo '</pre>';
         ...
         $tweet['like_count'] = $like_cnt['like_count'];//<=必ず$aql分のテーブル名の名前と揃える
        ...
        <form method="post" action="" class="form-horizontal" role="form">
          ...
          <ul class="paging">
            <input type="submit" class="btn btn-info" value="つぶやく">
                ...
                <li>
                <?php if($page < $maxPage){ ?>
                <a href="index.php?page=<? echo $page +1; ?><?php echo $word; ?>" class="btn btn-default">次</a>
                <?php }else{ ?>
                次
                <?php } ?>
                </li>
         ...
         <?php 
          ...
          <p>
           ...
           <span class="name"> <?php echo $tweet_each['nick_name']; ?> </span>
            ...
            <?php if($tweet_each['like_count'] >= 1){ ?>
            ...
            <?php if($tweet_each['like_flag'] == 1){
                 //すでにいいねされているので　いいねを取り消す　を表示 <=php内にコメント書くとソースで公表されないので便利
                 ?>
               <a href="unlike.php?tweet_id=<?php echo $tweet_each['tweet_id']; ?>"><small>いいねを取り消す</small></a>
               <?php }else{//まだいいねされてないので　いいね　を表示?>
